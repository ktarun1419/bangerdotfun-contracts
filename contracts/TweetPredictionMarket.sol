...
contract LongToken is ERC20 {
    ...
    constructor(string memory tweetId) ERC20(
...
contract TweetPredictionMarket is ReentrancyGuard {
    ...
    // Events
    event TokensPurchased(address indexed buyer, bool isLong, uint256 tokens, uint256 cost);
    ...
    function calculateMintCost(uint256 totalSupply, uint256 tokens) public view returns (uint256) {
        // Coupled bonding curve: cost = a * tokens + b * (totalSupply * tokens + 0.5 * tokens^2)
        uint256 linearCost = (a * tokens) / 1e18;
        uint256 quadraticCost = (b * (totalSupply * tokens + (tokens * tokens) / 2)) / 1e36;
    ...
    function settle() external {
        ...
        require(block.timestamp >= settlementTime, "Settlement time not reached");
        ...
        if (longWon) {
            ...
            if (shortReserve > 0 && longSupply > 0) {
                uint256 rake = (shortReserve * SETTLE_RAKE_RATE) / FEE_PRECISION;
                protocolFees += rake;
    ...
    function getShortTokenBalance(address user) external view returns (uint256) {
